...
/**
 * Sinais pessoais (Personal Kanban) do colaborador autenticado: meu WIP e o
 * subconjunto envelhecendo/em risco (atrasado/vence logo). Reusa o MESMO cálculo
 * de aging do cockpit (DEFAULT_SLA_HOURS/AGING_ALERT_RATIO/stageAgingRatio),
 * escopado a `assigneeId = eu` — não exige admin, é o próprio dado.
 */
export async function getMyFocus(): Promise<MyFocus> {
  const user = await getSessionUser();
  const { getDueState } = await import("@/lib/dates");
  const { stageAgingRatio } = await import("@/lib/team-health-format");
  const now = Date.now();

  const stages = await prisma.taskActiveStage.findMany({
    where: { status: "ACTIVE", assigneeId: user.id },
    select: {
      activatedAt: true,
      task: { select: { id: true, title: true, dueDate: true } },
      stage: { select: { name: true, expectedDurationHours: true } },
  ...
  let dueSoon = 0;
  let aging = 0;
  const items: MyFocusItem[] = [];
  for (const s of stages) {
    const slaHours = s.stage.expectedDurationHours ?? DEFAULT_SLA_HOURS;
    const ageHours = (now - s.activatedAt.getTime()) / 3.6e6;
    const agingRatio = stageAgingRatio(s.activatedAt, slaHours, now);
    const dueState = getDueState(s.task.dueDate);
    if (dueState === "overdue") overdue++;
    else if (dueState === "dueSoon") dueSoon++;
    const isAging = agingRatio >= AGING_ALERT_RATIO;
    if (isAging) aging++;
    if (isAging || dueState !== "none") {
      items.push({
        taskId: s.task.id,
        taskTitle: s.task.title,
        stageName: s.stage.name,
  ...
  items.sort((a, b) => b.agingRatio - a.agingRatio);
  return { wip: stages.length, overdue, dueSoon, aging, items };
}
