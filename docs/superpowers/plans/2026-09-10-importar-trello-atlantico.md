...
**Goal:** trazer 204 demandas de 17 meses do quadro "Atlantico Shop" para o WorkOS, com as etapas que foram observadas, as datas históricas reais e nenhuma etapa inventada.
...
- **Etapa sem evidência é etapa NÃO-INCLUÍDA, nunca inferida.** Vale para toda a entrega. `Briefing`, `Gráfica`, `Trafego Pago`, `Imprensa` e `Relatório` ficam fora das 204 demandas.
...
- [ ] **Step 5: Verificar contra o export real** — um teste que lê o JSON e afirma a contagem: 204 demandas, 38 separadores, 25 ausências, 10 referências, 26 sem etapa (soma 303 — confira a soma, foi ela que pegou meu erro). Se o arquivo não estiver presente no ambiente, o teste é pulado com `it.skip` e uma nota; **não invente números**.
...
it("o relatório diz o que entra, o que fica de fora e por quê", () => {
  ...
  expect(txt).toContain("204 demandas");
  expect(txt).toContain("17 projetos");
...
Expected: relatório com 204 demandas, 17 projetos, 99 descartados e a lista de repescagem. **Nenhuma escrita** — confirme com uma contagem de demandas antes e depois.
...
- [ ] **Step 2:** registrar a decisão em aberto que a spec levantou: o **projeto que já existe** no cliente AtlanticoShop convive com os 17 mensais ou é absorvido.
