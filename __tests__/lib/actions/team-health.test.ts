...
describe("getMyFocus", () => {
  ...
  it("counts WIP/aging/overdue/dueSoon and returns the at-risk subset sorted by ratio", async () => {
    mockAuth.mockResolvedValue({ user: { id: "me", role: "MEMBER" } } as never);
    const hoursAgo = (h: number) => new Date(Date.now() - h * 3.6e6);
    const future = (d: number) => new Date(Date.now() + d * 864e5);
    db.taskActiveStage.findMany.mockResolvedValue([
      // 96h old, SLA 24 → ratio 4 (aging); due far
      {
        activatedAt: hoursAgo(96),
        task: { id: "t1", title: "A", dueDate: future(30) },
        stage: { name: "Dev", expectedDurationHours: 24 },
      },
      // fresh, no SLA (default 72) → not aging; overdue by date
      {
        activatedAt: hoursAgo(1),
        task: { id: "t2", title: "B", dueDate: new Date(Date.now() - 864e5) },
        stage: { name: "QC", expectedDurationHours: null },
      },
      // fresh, far due → not at risk → excluded from items
      {
        activatedAt: hoursAgo(1),
        task: { id: "t3", title: "C", dueDate: future(30) },
        stage: { name: "SEO", expectedDurationHours: 72 },
      },
    ] as never);

    const focus = await getMyFocus();
    expect(focus.wip).toBe(3);
    expect(focus.aging).toBe(1);
    expect(focus.overdue).toBe(1);
    expect(focus.dueSoon).toBe(0);
    expect(focus.items.map((i) => i.taskId)).toEqual(["t1", "t2"]); // t3 excluded; ratio desc
  });

  it("is empty when I have no active stages", async () => {
    mockAuth.mockResolvedValue({ user: { id: "me", role: "MEMBER" } } as never);
    db.taskActiveStage.findMany.mockResolvedValue([] as never);
    const focus = await getMyFocus();
    expect(focus).toMatchObject({ wip: 0, aging: 0, overdue: 0, dueSoon: 0, items: [] });
  });
});
